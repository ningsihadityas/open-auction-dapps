...
function testGetItemName() public {
    string memory returnedItemName = auction.getItemName(expectedaucId); 
...
function testGetItemPrice() public {
    uint returnedAuctionPrice = auction.getItemPrice(expectedaucId); 
...
function testGetItemIncrement() public {
    uint returnedItemIncrement = auction.getItemIncrement(expectedaucId); 
...
function testGetPercentIncrease() public {
    uint returnedPercentIncrease = auction.getPercentIncrease(expectedaucId); 
...
function testUserCanBid() public {
  uint returnedId = auction.placeBid(expectedaucId, expectedBidAmount);

  Assert.equal(returnedId, expectedaucId, "Bid is possible with value higher than increment.");
}


// The variables that we will test
uint constant expectedItemCount = 4; // expected item count
uint expectedaucId = 0; // expected id of art piece
